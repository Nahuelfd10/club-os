...
export default function AdminChargeDetailPage() {
  ...
  useEffect(() => {
    if (!charge || !/^(?:\/.+)?\/admin\/charges\/[^/]+$/.test(pathname)) {
      return;
    }

    const canonicalBasePath = charge.category === "membership" ? "charges/membership" : "charges/lists";
    router.replace(routes.adminPath(`${canonicalBasePath}/${charge.id}`));
  }, [charge, pathname, router, routes]);
  ...
  const fallbackBackPath = pathname.includes("/charges/lists/")
    ? routes.adminPath("charges/lists")
    : routes.adminPath("charges/membership");
  const fallbackBackLabel = pathname.includes("/charges/lists/")
    ? "Volver a listas de recaudacion"
    : "Volver a cuotas mensuales";
